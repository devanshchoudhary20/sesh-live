...
| CLI host agent | TypeScript on Node 22, `node-pty` via the prebuilt multiarch package, `ws`, `commander`, asciinema v2 cast writer, WebCrypto AES-GCM for E2E, `@xterm/headless` + `@xterm/addon-serialize` for the join snapshot (PM gate 1 fix, 2026-09-25), the viewer input filter and Ctrl+] prefix handler. Published as `sesh` on npm, plus a brew tap. | `npx` is the distribution channel developers already use for agent tooling. Bun single-binary is a later packaging change, not a rewrite. |
